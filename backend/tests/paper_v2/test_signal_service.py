...
カテゴリ:
  [A] 初期化 / artifact self-check (4 tests)
  [B] OOS 境界 (2 tests)
  ...
  [G] range 実行 (2 tests)
  [H] 冪等性 / 決定性 (2 tests)
  [I] constants / SkipReason カバレッジ (1 test)
"""
...
class TestLookaheadPrevention:
    def test_lookahead_prevention_build_window(
        ...
        get_prices_between の end 引数が us_signal_date - 1 日以内であること。
        """
        us_date = _US_DATE
        jp_date = _JP_DATE

        training_end = us_date - timedelta(days=1)
        ...
        # get_prices_between が呼ばれた場合、end が jp_date を超えていないこと
