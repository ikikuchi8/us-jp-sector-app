...
# パイプライン (1 jp_execution_date あたり)
  ...
  2. 訓練窓構築: us_signal_date を軸に L=60 営業日の US+JP 28 CC returns 完全観測行
...
# 先読み防止 (paper_v1 と同等水準)
  - 訓練窓の末尾: us_signal_date - 1 日 / jp_execution_date - 1 日
  - 現在 US リターン: as_of_date=us_signal_date のみ
...
class PaperV2SignalService:
    ...
    def _generate_for_date(
        ...
        # ── 訓練窓構築 ──
        rcc_window = self._build_window(us_date, jp_date)
    ...
    def _build_window(
        ...
        先読み防止:
          - 訓練末尾 = min(us_signal_date, jp_execution_date) - 1 日
          - jp_execution_date 当日は含めない

        Returns:
            (n_rows, 28) ndarray or None (データ不足)
        """
        # 先読み防止: 訓練末尾は us_signal_date - 1 日
        # (CalendarService により us_signal_date < jp_execution_date が保証されるため
        #  us_signal_date - 1 が jp_execution_date - 1 より常に早い)
        training_end = us_signal_date - timedelta(days=1)
        buffer_start = us_signal_date - timedelta(days=_FETCH_BUFFER_DAYS)
        ...
        # ------------------------------------------------------------------
        # バッチ取得: JP 価格 (CC リターン用: adjusted_close_price を使用)
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # 訓練行の構築 (complete-case: US 28 + JP 17 全て揃っている行のみ)
        # ------------------------------------------------------------------
    ...
    def _fetch_current_us_returns(self, us_signal_date: date) -> np.ndarray | None:
        ...
        Returns:
            US CC リターンベクトル (N_US,) or None (データ不足時)
        """
        x_vals: list[float] = []
